...
public class CameraController6 : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        Physics.Linecast(player.transform.position, transform.position, out hit);
        if (hit.transform != null)
        {
            if (hit.transform != transform)
            {
                ...
                transform.position = new Vector3(player.transform.position.x, transform.position.y, hit.transform.position.z);
            }
        }
    }
}
